...
public class BMountains : Biome
{
    ...
    public int _maxSurfaceheight = Constants.MAX_HEIGHT - 1;

    [Header("Texture generation")] [Tooltip("Increase the effect of the hightMatMult")] [Range(1, 20f)]
    public float _heightMatOffset = 10;

    [Tooltip("Multiplier of the slope in dependence of the height")]
    public AnimationCurve _hightMatMult;

    [Tooltip("Height where the grass change to snow")] [Range(0, Constants.MAX_HEIGHT)]
    public int _snowHeight = 35;

    [Tooltip("Slope vale where terrain start to be rock")] [Range(0, 1f)]
    public float _rockLevel = 0.6f;

    [Tooltip("Slope vale where terrain start to be dirt")] [Range(0, 1f)]
    public float _dirtLevel = 0.25f;


    public override byte[] GenerateChunkData(Vector2Int vecPos, float[] biomeMerge)
    {
        int surfaceStart = NoiseManager.Instance._worldConfig._surfaceLevel; //Avoid too high value that generate bad mesh
        byte[] chunkData = new byte[Constants.CHUNK_BYTES];
        float[] noise = NoiseManager.GenerateExtendedNoiseMap(_scale, _octaves, _persistance, _lacunarity, vecPos);
        for (int z = 0; z < Constants.CHUNK_VERTEX_SIZE; z++) //start a 1 because the noise start at -1 of the chunk vertex
        {
            for (int x = 0; x < Constants.CHUNK_VERTEX_SIZE; x++) //start a 1 because the noise start at -1 of the chunk vertex
            {
                ...
                float height = Mathf.Lerp(
                    ...
                    (_terrainHeightCurve.Evaluate(noise[(x + 1) + (z + 1) * (Constants.CHUNK_VERTEX_SIZE + 2)]) * (_maxSurfaceheight - surfaceStart) + surfaceStart), //Desired biome height
                ...
                for (int y = 0; y < Constants.CHUNK_VERTEX_HEIGHT; y++)
                {
                    ...
                    else if (y < heightY)
                    {
                        ...
                        if (slope > _rockLevel)
                            chunkData[index + 1] = 4; //Rock
                        else if (slope < _dirtLevel && y > _snowHeight) //Avoid dirt in snow areas
                            chunkData[index + 1] = 3;
                        else
    ...
    private float CalculateSlope(int x, int z, float[] noise)
    {
        ...
        for (int xOffset = x - 1; xOffset <= x + 1; xOffset += 1)
        {
            ...
            {
                float value = _terrainHeightCurve.Evaluate(noise[xOffset + zOffset * (Constants.CHUNK_VERTEX_SIZE + 2)]);
                if (value < minValue)
                    minValue = value;
            }
        }

        float pointValue = _terrainHeightCurve.Evaluate(noise[x + z * (Constants.CHUNK_VERTEX_SIZE + 2)]);
        return (1 - (minValue / pointValue)) * (_hightMatMult.Evaluate(pointValue) * _heightMatOffset);
        ;
    }
}