...
{
    [Header("World configuration")] public WorldConfig _worldConfig; //Current world configuration of the noiseManager

    [System.Serializable]
    public class WorldConfig
    {
        [Tooltip("Seed of the world (0 seed generate randoms different worlds -testing-)")] [Range(int.MinValue + 100, int.MaxValue - 100)]
        public int _worldSeed = 0;

        [Tooltip("Biomes sizes")] public float _biomeScale = 150;

        [Header("Biome merge configuration")] [Tooltip("biomes.appearValue difference for merge")] [Range(0.01f, 0.5f)]
        public float _diffToMerge = 0.025f;

        [Tooltip("Surface desired level, height where biomes merge")] [Range(1, Constants.MAX_HEIGHT)]
        public int _surfaceLevel = Constants.MAX_HEIGHT / 8;
        ...
        public float _persistance = 0.1f;

        [Tooltip("Frequency increase of biomes per octave")] [Range(1, 20)]
        public float _lacunarity = 9f;
    }


    [Header("Biomes Array")] // Empty for get all Biomes of inside the GameObject
    public BiomeProperties[] _biomes;

    [System.Serializable]
    public struct BiomeProperties
    {
        public Biome _biome; //Biome child class
        public float _appearValue; //1 to 0 value when the biome appears. Next biomePropertie.appear value is where this biome end
    }

    public void Start()
    {
        if (_worldConfig._worldSeed == 0 && !WorldManager.IsCreated()) //Generate random seed when use 0 and is scene testing (no WorldManager exists)
        {
            ...
            WorldManager.CreateWorld(selectedWorld, _worldConfig); //Create a new world folder for correct working
            _worldConfig._worldSeed = Random.Range(int.MinValue, int.MaxValue);
        }
        else if ((Constants.AUTO_CLEAR_WHEN_NOISE_CHANGE) && !WorldManager.IsCreated()) //If AUTO_CLEAR_WHEN_NOISE_CHANGE true and world manager not exist, we clear old world data (we assume we are using a debug scene)
        {
            ...
            if (loadedWorldConfig._worldSeed != _worldConfig._worldSeed || loadedWorldConfig._biomeScale != _worldConfig._biomeScale || loadedWorldConfig._diffToMerge != _worldConfig._diffToMerge || loadedWorldConfig._surfaceLevel != _worldConfig._surfaceLevel ||
                loadedWorldConfig._octaves != _worldConfig._octaves || loadedWorldConfig._persistance != _worldConfig._persistance || loadedWorldConfig._lacunarity != _worldConfig._lacunarity)
            {
                WorldManager.DeleteWorld(selectedWorld); //Remove old world
                WorldManager.CreateWorld(selectedWorld, _worldConfig); //Create new world with the new worldConfig
            }
        }
        else if (WorldManager.IsCreated()) //Load config of the world
        {
            _worldConfig = WorldManager.GetSelectedWorldConfig();
        }

        if (_biomes.Length == 0)
        {
            Biome[] biomeArray = GetComponents<Biome>();
            _biomes = new BiomeProperties[biomeArray.Length];
            for (int i = 0; i < biomeArray.Length; i++)
            {
                _biomes[i]._biome = biomeArray[i];
                _biomes[i]._appearValue = (float)(biomeArray.Length - i) / biomeArray.Length;
            }
        }
    ...
    public byte[] GenerateChunkData(Vector2Int vecPos)
    {
        ...
        float[] biomeNoise = GenerateNoiseMap(_worldConfig._biomeScale * _biomes.Length, _worldConfig._octaves, _worldConfig._persistance, _worldConfig._lacunarity, vecPos); //Biomes noise (0-1) of each (x,z) position
        ...
        byte[][] biomesData = new byte[_biomes.Length][]; //Data generate from biomes.biome.GenerateChunkData()
        ...
        for (int x = 0; x < Constants.CHUNK_VERTEX_SIZE; x++)
        {
            for (int z = 0; z < Constants.CHUNK_VERTEX_SIZE; z++)
            {
                ...
                {
                    biomesData[biomeTable[index]] = _biomes[biomeTable[index]]._biome.GenerateChunkData(vecPos, mergeBiomeTable);
                }
    ...
    private int[] GetChunkBiomes(float[] noise, out float[] mergeBiome)
    {
        ...
        for (int z = 0; z < Constants.CHUNK_VERTEX_SIZE; z++)
        {
            for (int x = 0; x < Constants.CHUNK_VERTEX_SIZE; x++)
            {
                ...
                {
                    if (noise[index] < _biomes[i]._appearValue)
                    {
                        if (i != 0 && _worldConfig._diffToMerge + noise[index] > _biomes[i]._appearValue) //Biome merged with top biome
                        {
                            mergeBiomeTable[index] = (_biomes[i]._appearValue - noise[index]) / _worldConfig._diffToMerge;
                            //Debug.Log("TOP: "+biomes[i].appearValue + " - " + noise[index] + " / " + diffToMerge + " = " + mergeBiomeTable[index]);
                        }

                        else if (i != _biomes.Length - 1 && _worldConfig._diffToMerge - noise[index] < _biomes[i + 1]._appearValue) //Biome merged with bottom biome
                        {
                            mergeBiomeTable[index] = (noise[index] - _biomes[i + 1]._appearValue) / _worldConfig._diffToMerge;
    ...
    public static float[] GenerateNoiseMap(float scale, int octaves, float persistance, float lacunarity, Vector2Int offset)
    {
        ...
        System.Random random = new System.Random(Instance._worldConfig._worldSeed); //Used System.random, because unity.Random is global, can cause problems if there is other random running in other script
    ...
    public static float[] GenerateExtendedNoiseMap(float scale, int octaves, float persistance, float lacunarity, Vector2Int offset)
    {
        ...
        System.Random random = new System.Random(NoiseManager.Instance._worldConfig._worldSeed); //Used System.random, because unity.Random is global, can cause problems if there is other random running in other script
        Vector2[] octaveOffsets = new Vector2[octaves];
